...
_RE_MUSIC = re.compile(
    r"(concert|live music|music hall|goldenvoice|popscene|dj\b|album release|band\b|tour\b)",
    re.IGNORECASE,
)
_RE_FESTIVAL = re.compile(
    r"(festival|parade|carnaval|celebration|fair\b|holiday)", re.IGNORECASE
)
_RE_ART = re.compile(
    r"(art\b|poetry|gallery|performance project|installation)", re.IGNORECASE
)
...
def normalize_push_preferences(prefs: PushPreferences | dict | None) -> PushPreferences:
    ...
    return PushPreferences(
        neighborhoods=neighborhoods, cuisines=cuisines, event_categories=categories
    )
...
def derive_event_category(event: SFEvent | dict) -> EventCategory:
    if isinstance(event, SFEvent):
        title, location, description = event.title, event.location, event.description or ""
    else:
...
def group_by_neighborhood(
    restaurants: list[Restaurant], events: list[SFEvent] | None = None
    ...
    for e in events or []:
        key = derive_event_neighborhood(e)
